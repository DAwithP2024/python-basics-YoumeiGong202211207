...
def display_sorted_products(products_list, sort_order):
    sorted_products = sorted(products_list.items(), key=lambda x: x[1], reverse=(sort_order == 'desc'))
    print("\nSorted Products:")
    for idx, (product, price) in enumerate(sorted_products, 1):
        print(f"{idx}. {product} - ${price:.2f}")

def add_to_cart(cart, product, quantity):
    # Check if the product is already in the cart and update the quantity if so
    for idx, (cart_product, cart_quantity) in enumerate(cart):
        if cart_product == product:
            cart[idx] = (cart_product, cart_quantity + quantity)
            return
